...
void menu_advanced_settings() {
  ...
  BACK_ITEM(MSG_CONFIGURATION);
      ...
      if (!is_busy) {
        ...
        EDIT_ITEM_FAST(float51sign, MSG_DRAW_MIN_Y, &draw_area_min.y, Y_MIN_POS -10, draw_area_max.y); // M665 T
        EDIT_ITEM_FAST(float51sign, MSG_DRAW_MAX_Y, &draw_area_max.y, draw_area_min.y + 10, Y_MAX_POS); // M665 B
        EDIT_ITEM_FAST(float51sign, MSG_MAX_BELT_LEN, &polargraph_max_belt_len, 500, 3000);             // M665 H
      }
    #endif
